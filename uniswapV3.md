...
为了限制区间范围，减少cross tick bit的次数，添加了 tickSpacing。 这个根据费率限制， 0.01就是1， 0.05就是30这样， 就是tick只能是tickSpacing的整数倍。中间的设置像整数倍靠拢。

#### liquidityNet
这个概念就是， 一个tick区间比如[1500,1550]这个价格区间， 可能有N个池子覆盖这个区间，需要统计这个池子的真实流动性。
价格上涨，当经过Lower的时候加流动性，经过Upper的减流动性。 下跌相反。
每个tick在添加的时候就添加lower和upper应该有加减多少流动性。 当价格扫到这个tick的时候，先对流动性进行处理。 因为全局维护一个流动性，所以流动性的维护异常简单。

#### cross tick
当一个区间的价格顶不住的时候，自然会像左或者右去找下一个tick。 此时就要重新更新流动性，及计算该区间能换多少钱。 
详细流程：
